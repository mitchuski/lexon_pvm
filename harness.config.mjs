...
2. Build the uncovered list: run \`node ${ctx.repo}/tools/coverage.mjs --json\`, uncovered = CENSUS.json term ids minus coveredIds minus the proposal's own censusIds, in census order. Covered list = coveredIds in census order (if fewer than 3 covered entries exist yet, draw what exists and say so; the regression slots shrink accordingly and gateResult denominator with them).
...
2. Hard constraint: every entry has cites resolving to a SOURCES.md slug, a registerTag consistent with the canon source (spot-check 2 against their census anchors), no em dash anywhere in the proposal.
          ...
          items: {
            ...
            properties: {
              ...
              entries: {
                ...
                items: {
                  type: 'object', required: ['censusId', 'term', 'lexText', 'cites', 'registerTag'],
                  properties: {
